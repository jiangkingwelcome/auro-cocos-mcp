...
function extractPart(fullStr, startMarker, endMarker) {
    let startIndex = fullStr.indexOf(startMarker);
    if (startIndex === -1) throw new Error("Could not find start marker: " + startMarker);
    startIndex += startMarker.length;
    let endIndex = fullStr.indexOf(endMarker, startIndex);
    if (endIndex === -1) throw new Error("Could not find end marker");
    return { startIndex, endIndex, oldStr: fullStr.substring(startIndex, endIndex) };
}

// Extract and replace template
let result = extractPart(content, '  template: /* html */ \`\n', '\n  \`,');
content = content.substring(0, result.startIndex) + newTemplate + content.substring(result.endIndex);

// Extract and replace css
let result2 = extractPart(content, '  style: /* css */ \`\n', '\n  \`,');
content = content.substring(0, result2.startIndex) + newCSS + content.substring(result2.endIndex);
