(() => {
  ...
  const STORAGE_KEY = "auro-cocos-interactive-test-plan-v1";
  const STATUS_LABEL = {
    pending: "待测",
    pass: "通过",
    fail: "失败",
  ...
  function init() {
    dom.heroDesc.textContent =
      `内置现有 ${DATA.project.totalCases} 条测试用例，并预填 tests/test-report.json 中的 AI 自动化基线。你可以继续补人工结果、记录问题、指定责任人、标记复测，并导出当前测试状态。`;
  ...
  function loadState() {
    ...
    DATA.cases.forEach((item) => {
      ...
      cases[item.id] = {
        aiStatus: old.aiStatus || item.aiBaseline.status || "pending",
        manualStatus: old.manualStatus || "pending",
        severity: old.severity || "none",
        retestStatus: old.retestStatus || "pending",
        owner: old.owner || "",
        bugId: old.bugId || "",
        aiNote: old.aiNote !== undefined ? old.aiNote : item.aiBaseline.note || "",
        manualNote: old.manualNote || "",
        issue: old.issue || "",
      ...
      <details class="case">
        <summary>
          <div class="case-head">
            ...
            <span>${badge(item.priority, "priority")}${badge(EDITION_LABEL[item.edition] || item.edition, item.edition)}<span class="badge">${esc(item.phase)}</span>${badge(`AI：${STATUS_LABEL[row.aiStatus]}`, row.aiStatus)}${badge(`人工：${STATUS_LABEL[row.manualStatus]}`, row.manualStatus)}${badge(`总体：${STATUS_LABEL[overall]}`, overall)}</span>
          </div>
        ...
        <div class="case-body">
          ...
          <section class="case-grid">
            ...
            ${fieldArea(item.id, "manualNote", "人工备注", row.manualNote, "人工验证步骤、截图位置、操作结论")}
  ...
  function computeStats() {
    const stats = {
      ...
      ai: { pass: 0, fail: 0, blocked: 0, pending: 0 },
      manual: { pass: 0, fail: 0, blocked: 0, pending: 0 },
      manualDone: 0,
      dualComplete: 0,
      dualPass: 0,
      openIssues: 0,
    ...
    DATA.cases.forEach((item) => {
      ...
      stats.ai[row.aiStatus] += 1;
      stats.manual[row.manualStatus] += 1;
      ...
      if (row.issue.trim() || row.aiStatus === "fail" || row.manualStatus === "fail" || row.severity !== "none") {
        stats.openIssues += 1;
      }
    });
