...
- 3 个原子宏: create_prefab_atomic, import_and_apply_texture, auto_fit_physics_collider
- execute_script, register_custom_macro
- physics_tool (10 actions)
- 动画查询保留在 `scene_query.get_animation_state`

`setup_ui_layout` 与 `create_ui_widget` 已从社区版 JS 注册链移除，仅保留为 Pro 能力。

**社区版合计: 174 actions（以 docs/business/trust-metrics.json 为准）**

---

## 三、Pro 版功能清单 (商业承诺 270+；Native 注册最高 412，持续增长)

### 社区版全部 actions 的 Rust 重写

Pro 版包含社区版全部 174 个 action 对应能力，但实现在 Rust 中，并额外解锁动画操作与动画原子宏。
...
- Native 代码注册快照：28 个工具，412 actions（含 Phase 4-5）
- 统一来源：`docs/business/PRODUCT_TRUST_ASSET.md` + `docs/business/trust-metrics.json`
...
| 层级 | 价格 | 内容 |
|------|------|------|
| **Community** | 免费 | 174 actions, JS 实现, GitHub 开源 |
