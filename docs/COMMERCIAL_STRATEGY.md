...
- 5 个原子宏: create_prefab_atomic, import_and_apply_texture, setup_ui_layout, create_tween_animation_atomic, auto_fit_physics_collider
...
## 三、Pro 版功能清单 (270+ actions, 持续增长)

### 社区版全部 actions 的 Rust 重写

Pro 版包含社区版全部 ~158 actions，但实现在 Rust 中。
...
| 功能 | Actions | 状态 | 说明 |
|------|---------|------|------|
| **AI 场景生成器** (`scene_generator`) | 5 | 已实现 | 自然语言 → 完整场景 (create_scene, create_ui_page, create_game_level, create_menu, describe_intent) |
| **批量操作引擎** (`batch_engine`) | 10 | 已实现 | 节点匹配 + 批量变换 (find_and_modify/delete/add_component/remove_component/set_property/reparent, transform_all, rename_pattern, set_layer_recursive, toggle_active_recursive) |
| **场景审计工具** (`scene_audit`) | 9 | 已实现 | 自动检测问题 + 修复建议 (full_audit, check_performance/hierarchy/components/assets/physics/ui, auto_fix, export_report) |
...
**Pro 版合计: 22 个工具, 270+ actions + AI 智能工具，持续增长**
...
| 层级 | 价格 | 内容 |
|------|------|------|
| **Community** | 免费 | ~158 actions, JS 实现, GitHub 开源 |
